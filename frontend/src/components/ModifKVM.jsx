...
// --- Composant principal ---
const ModifKVM = ({ isOpenFormModifKVM, closeFormModifKVM, kvm, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess("KVM modifié avec succès !");
  ...
  if (!isOpenFormModifKVM) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un KVM</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifKVM}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={kvm._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={kvm.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={kvm.modele} required />
          <FloatingInput label="N° Série" name="noSerie" defaultValue={kvm.noSerie} required />
          <FloatingInput label="Date d’acquisition" name="dateAchatNonFormatted" type="date" defaultValue={kvm.dateAchat} />

          {/* Etat */}
          <FloatingSelect label="État" name="etat" required defaultValue={kvm.etat}>
          ...
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===kvm.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={kvm.commentaire} />
        </div>
