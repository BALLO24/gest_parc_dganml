...
// --- Composant principal ---
const ModifSwitch = ({ isOpenFormModifSwitch, closeFormModifSwitch, switchs, onSuccess }) => {
  ...
  if (!isOpenFormModifSwitch) return null;
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={switchs._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={switchs.marque} required />
          ...
          <FloatingInput label="Nombre de port" name="nbrePort" defaultValue={switchs.nbrePort} required />

          <FloatingSelect label="POE" name="poe" required defaultValue={switchs.poe}>
            <option value="">---</option>
            <option value="oui">Oui</option>
            <option value="non">Non</option>
          </FloatingSelect>
            
            <FloatingSelect label="Manageable" name="manageable" required defaultValue={switchs.manageable}>
                <option value="">---</option>
                <option value="oui">Oui</option>
                <option value="non">Non</option>
          </FloatingSelect>
          ...
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===switchs.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={switchs.commentaire} />
        </div>
