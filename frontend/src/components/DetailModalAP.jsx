...
const DetailModalAP = ({ isOpenModalInfo, onCloseModalInfo, ap}) => {
    ...
    >
      ...
      >
        ...
        <div className="p-6 text-gray-700 text-sm">
          <p><span className="font-semibold">Marque : </span>{ap.marque}</p>
          <p><span className="font-semibold">Modele : </span>{ap.modele}</p>
          <p><span className="font-semibold">Numero de série : </span>{ap.noSerie}</p>
          <p><span className="font-semibold">Date d'acquisition : </span>{ap.dateAchat}</p>
          <p><span className="font-semibold">Etat : </span>{ap.etat}</p>
          <p><span className="font-semibold">Utilisateur actuel : </span>{ap.userActuel.prenom} {ap.userActuel.nom}</p>
          <p><span className="font-semibold">Site actuel : </span>{ap.userActuel.site.nom}</p>
          <p><span className="font-semibold">Autres infos : </span>{ap.commentaire}</p>
          <div className="mt-4">
            <span className="font-semibold">Historique :</span>
            {ap.historique.length !== 0 ? (
              <table className="min-w-full mt-2 text-sm border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                  {ap.historique.map((element, index) => (
