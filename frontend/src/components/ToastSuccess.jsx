...
export default function ToastSuccess({ message = "Opération réussie !"}) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 rounded-lg h-20 bg-white/95 px-4 py-4 shadow-lg border border-green-200 text-green-700">
        <span className="flex-1 text-md font-semibold ">{message}</span>

        <button
          onClick={startClose}
          aria-label="Fermer la notification"
          className="inline-flex h-7 w-7 items-center justify-center rounded-md text-green-600 hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-200"
        >
          ✕
        </button>
      </div>
