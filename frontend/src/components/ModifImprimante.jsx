...
// --- Composant principal ---
const ModifImprimante = ({ isOpenFormModifImprimante, closeFormModifImprimante, imprimante, onSuccess }) => {
  ...
  if (!isOpenFormModifImprimante) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un imprimante</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifImprimante}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={imprimante._id} />

          {/* Type imprimante */}
          <FloatingSelect label="Type d'imprimante" name="typeImprimante" required defaultValue={imprimante.typeImprimante}>
            {data.typeImprimante.map((el,index) => (
        ...
        <FloatingInput label="Marque" name="marque" defaultValue={imprimante.marque} required />
          ...
          <FloatingSelect label="Couleur" name="couleur" required defaultValue={imprimante.couleur}>
            <option value="">---</option>
            <option value="Monochrome">Monochrome</option>
            <option value="En Couleur">En couleur</option>
          </FloatingSelect>

          <FloatingSelect label="Type Bac" name="typeBac" required defaultValue={imprimante.typeBac}>
            <option value="">---</option>
          ...
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===imprimante.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={imprimante.commentaire} />
        </div>
...
export default ModifImprimante;
