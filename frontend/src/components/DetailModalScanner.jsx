...
const DetailModalSanner = ({ isOpenModalInfo, onCloseModalInfo, scanner}) => {
    ...
    >
      ...
      >
        ...
        <div className="p-6 text-gray-700 text-sm">
          <p><span className="font-semibold">Marque : </span>{scanner.marque}</p>
          <p><span className="font-semibold">Modele : </span>{scanner.modele}</p>
          <p><span className="font-semibold">Bac : </span>{scanner.typeBac}</p>
          <p><span className="font-semibold">Recto verso : </span>{scanner.rectoVerso}</p>
          <p><span className="font-semibold">Numero de série : </span>{scanner.noSerie}</p>
          <p><span className="font-semibold">Date d'acquisition : </span>{scanner.dateAchat}</p>
          <p><span className="font-semibold">Etat : </span>{scanner.etat}</p>
          <p><span className="font-semibold">Utilisateur actuel : </span>{scanner.userActuel.prenom} {scanner.userActuel.nom}</p>
          <p><span className="font-semibold">Utilisateur site : </span>{scanner.userActuel.site.nom}</p>
          <p><span className="font-semibold">Autres infos : </span>{scanner.commentaire}</p>
          <div className="mt-4">
            <span className="font-semibold">Historique :</span>
            {scanner.historique.length !== 0 ? (
              <table className="min-w-full mt-2 text-sm border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                  {scanner.historique.map((element, index) => (
...
export default DetailModalSanner;
