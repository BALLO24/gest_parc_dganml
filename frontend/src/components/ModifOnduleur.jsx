...
// --- Composant principal ---
const ModifOnduleur = ({ isOpenFormModifOnduleur, closeFormModifOnduleur, onduleur, onSuccess }) => {
  ...
  if (!isOpenFormModifOnduleur) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un onduleur</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifOnduleur}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FloatingSelect label="Type" name="typeOnduleur" required defaultValue={onduleur.typeOnduleur}>
            <option value="">---</option>
            <option value="Mini-onduleur">Mini-onduleur</option>
            <option value="Grande Capacité">Grande Capacité</option>
          </FloatingSelect>
          <FloatingInput label="Puissance" name="puissance" defaultValue={onduleur.puissance} required />
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===onduleur.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={onduleur.commentaire} />
        </div>
