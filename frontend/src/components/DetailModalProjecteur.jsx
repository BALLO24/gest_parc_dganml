...
const DetailModalProjecteur = ({ isOpenModalInfo, onCloseModalInfo, projecteur}) => {
    ...
    >
      ...
      >
        ...
        <div className="p-6 text-gray-700 text-sm">
          <p><span className="font-semibold">Marque : </span>{projecteur.marque}</p>
          <p><span className="font-semibold">Modele : </span>{projecteur.modele}</p>
          <p><span className="font-semibold">Numero de série : </span>{projecteur.noSerie}</p>
          <p><span className="font-semibold">Qualité d'image : </span>{projecteur.imgQualite}</p>
          <p><span className="font-semibold">Date d'acquisition : </span>{projecteur.dateAchat}</p>
          <p><span className="font-semibold">Etat : </span>{projecteur.etat}</p>
          <p><span className="font-semibold">Utilisateur actuel : </span>{projecteur.userActuel.prenom} {projecteur.userActuel.nom}</p>
          <p><span className="font-semibold">Site actuel : </span>{projecteur.userActuel.site.nom}</p>
          <p><span className="font-semibold">Autres infos : </span>{projecteur.commentaire}</p>
          <div className="mt-4">
            <span className="font-semibold">Historique :</span>
            {projecteur.historique.length !== 0 ? (
              <table className="min-w-full mt-2 text-sm border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                  {projecteur.historique.map((element, index) => (
...
export default DetailModalProjecteur;
