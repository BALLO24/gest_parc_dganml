// NouveauTypeMateriel.jsx modernisé avec FloatingInput et labels flottants
...
const NouveauTypeMateriel = ({ isOpenFormNouveauTypeMateriel, closeFormNouveauTypeMateriel, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await API.addTypeMateriel(data);
      onSuccess("Nouveau type de materiel ajouté avec succès !");
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300" aria-modal="true" role="dialog">
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Ajouter un nouveau type</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FloatingInput label="Nom du type de matériel" name="nom" required />
        </div>
