...
// --- Composant principal ---
const ModifProjecteur = ({ isOpenFormModifProjecteur, closeFormModifProjecteur, projecteur, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess("Projecteur modifié avec succès !");
  ...
  if (!isOpenFormModifProjecteur) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un switch</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifProjecteur}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={projecteur._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={projecteur.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={projecteur.modele} required />
          <FloatingInput label="N° Série" name="noSerie" defaultValue={projecteur.noSerie} required />

          <FloatingInput label="Date d’acquisition" name="dateAchat" type="date" defaultValue={projecteur.dateAchat} required />

          {/* Etat */}
          <FloatingSelect label="État" name="etat" required defaultValue={projecteur.etat}>
          ...
          <FloatingSelect label="Qualité Image" name="imgQualite" required defaultValue={projecteur.imgQualite}>
            <option value="">---</option>
          ...
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===projecteur.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={projecteur.commentaire} />
        </div>
...
export default ModifProjecteur;
