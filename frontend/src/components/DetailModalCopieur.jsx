...
const DetailModalCopieur = ({ isOpenModalInfo, onCloseModalInfo, copieur}) => {
    ...
    >
      ...
      >
        ...
        <div className="p-6 text-gray-700 text-sm">
          <p><span className="font-semibold">Marque : </span>{copieur.marque}</p>
          <p><span className="font-semibold">Modele : </span>{copieur.modele}</p>
          <p><span className="font-semibold">Recto verso : </span>{copieur.rectoVerso}</p>
          <p><span className="font-semibold">Couleur : </span>{copieur.couleur}</p>
          <p><span className="font-semibold">Numero de série : </span>{copieur.noSerie}</p>
          <p><span className="font-semibold">Date d'acquisition : </span>{copieur.dateAchat}</p>
          <p><span className="font-semibold">Etat : </span>{copieur.etat}</p>
          <p><span className="font-semibold">Utilisateur actuel : </span>{copieur.userActuel.prenom} {copieur.userActuel.nom}</p>
          <p><span className="font-semibold">Site actuel : </span>{copieur.userActuel.site.nom}</p>
          <p><span className="font-semibold">Autres infos : </span>{copieur.commentaire}</p>
          <div className="mt-4">
            <span className="font-semibold">Historique :</span>
            {copieur.historique.length !== 0 ? (
              <table className="min-w-full mt-2 text-sm border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                  {copieur.historique.map((element, index) => (
...
export default DetailModalCopieur;
