...
// --- Composant principal ---
const ModifAntenne = ({ isOpenFormModifAntenne, closeFormModifAntenne, antenne, onSuccess }) => {
  ...
  if (!isOpenFormModifAntenne) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier une antenne</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifAntenne}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={antenne._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={antenne.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={antenne.modele} required />
          <FloatingInput label="N° Série" name="noSerie" defaultValue={antenne.noSerie} required />
          <FloatingInput type="number" label="Diamètre (cm)" name="diametre" defaultValue={antenne.diametre} required />
          <FloatingInput label="Date d’acquisition" name="dateAchatNonFormatted" type="date" defaultValue={antenne.dateAchat} />

          {/* Etat */}
          <FloatingSelect label="État" name="etat" required defaultValue={antenne.etat}>
          ...
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===antenne.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={antenne.commentaire} />
        </div>
