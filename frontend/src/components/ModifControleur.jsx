...
// --- Composant principal ---
const ModifControleur = ({ isOpenFormModifControleur, closeFormModifControleur, controleur, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess("Controleur modifié avec succès !");
  ...
  if (!isOpenFormModifControleur) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un Controleur</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifControleur}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={controleur._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={controleur.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={controleur.modele} required />
          <FloatingInput label="N° Série" name="noSerie" defaultValue={controleur.noSerie} required />

          <FloatingInput label="Date d’acquisition" name="dateAchat" type="date" defaultValue={controleur.dateAchat} required />

          {/* Etat */}
          <FloatingSelect label="État" name="etat" required defaultValue={controleur.etat}>
          ...
          <FloatingInput label="Nombre de port" name="nbrePort" defaultValue={controleur.nbrePort} required />

          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===controleur.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={controleur.commentaire} />
        </div>
...
export default ModifControleur;
