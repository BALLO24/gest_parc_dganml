...
// --- Composant principal ---
const ModifAP = ({ isOpenFormModifAP, closeFormModifAP, ap, onSuccess }) => {
  ...
  if (!isOpenFormModifAP) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un AP</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifAP}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={ap._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={ap.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={ap.modele} required />
          <FloatingInput label="N° Série" name="noSerie" defaultValue={ap.noSerie} required />
          <FloatingInput label="Date d’acquisition" name="dateAchatNonFormatted" type="date" defaultValue={ap.dateAchat} />

          {/* Etat */}
          <FloatingSelect label="État" name="etat" required defaultValue={ap.etat}>
          ...
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===ap.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={ap.commentaire} />
        </div>
