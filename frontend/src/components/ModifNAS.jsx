...
// --- Composant principal ---
const ModifNAS = ({ isOpenFormModifNAS, closeFormModifNAS, nas, onSuccess }) => {
  ...
  if (!isOpenFormModifNAS) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un NAS</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifNAS}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={nas._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={nas.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={nas.modele} required />
          <FloatingInput label="N° Série" name="noSerie" defaultValue={nas.noSerie} required />
          <FloatingInput label="Date d’acquisition" name="dateAchatNonFormatted" type="date" defaultValue={nas.dateAchat} />

          {/* Etat */}
          <FloatingSelect label="État" name="etat" required defaultValue={nas.etat}>
          ...
          {/* <FloatingInput label="Interface graphique" name="intGraphique" defaultValue={nas.intGraphique} required /> */}
          <FloatingInput type="number" label="Nombre de slots de disque" name="nbreSlotDisque" defaultValue={nas.nbreSlotDisque} />

          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===nas.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={nas.commentaire} />
        </div>
