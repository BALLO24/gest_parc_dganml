...
// --- Composant principal ---
const ModifRouteur = ({ isOpenFormModifRouteur, closeFormModifRouteur, routeur, onSuccess }) => {
  ...
  if (!isOpenFormModifRouteur) return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Modifier un routeur</h3>

        {/* Bouton fermer */}
        <button
          type="button"
          onClick={closeFormModifRouteur}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input type="hidden" name="_id" value={routeur._id} />


          {/* Marque */}
          <FloatingInput label="Marque" name="marque" defaultValue={routeur.marque} required />

          <FloatingInput label="Modèle" name="modele" defaultValue={routeur.modele} required />
          ...
          <FloatingInput label="Nombre de port" name="nbrePort" defaultValue={routeur.nbrePort} required />
          <FloatingSelect label="Utilisateur actuel" name="userActuel" required disabled>
            {agents.map((agent, index) => (
              <option key={index} value={agent._id} selected={agent._id===routeur.userActuel._id}>
                {agent.prenom} {" "} {agent.nom}
              </option>
            ))}
          </FloatingSelect>

          <FloatingTextarea label="Autres informations" name="commentaire" defaultValue={routeur.commentaire} />
        </div>
