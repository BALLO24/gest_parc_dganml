...
const DetailModalControleur = ({ isOpenModalInfo, onCloseModalInfo, controleur}) => {
    ...
    >
      ...
      >
        ...
        <div className="p-6 text-gray-700 text-sm">
          <p><span className="font-semibold">Marque : </span>{controleur.marque}</p>
          <p><span className="font-semibold">Modele : </span>{controleur.modele}</p>
          <p><span className="font-semibold">Nombre de port : </span>{controleur.nbrePort}</p>
          <p><span className="font-semibold">Numero de série : </span>{controleur.noSerie}</p>
          <p><span className="font-semibold">Date d'acquisition : </span>{controleur.dateAchat}</p>
          <p><span className="font-semibold">Etat : </span>{controleur.etat}</p>
          <p><span className="font-semibold">Utilisateur actuel : </span>{controleur.userActuel.prenom} {controleur.userActuel.nom}</p>
          <p><span className="font-semibold">Site actuel : </span>{controleur.userActuel.site.nom}</p>
          <p><span className="font-semibold">Autres infos : </span>{controleur.commentaire}</p>
          <div className="mt-4">
            <span className="font-semibold">Historique :</span>
            {controleur.historique.length !== 0 ? (
              <table className="min-w-full mt-2 text-sm border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                  {controleur.historique.map((element, index) => (
...
export default DetailModalControleur;
