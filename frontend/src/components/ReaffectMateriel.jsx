...
const ReaffectMateriel = ({ isOpenModalInfo, onCloseModalInfo, materiel, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess("Materiel réaffecté avec succès !");
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300" aria-modal="true" role="dialog">
      ...
      >
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Réaffecter un materiel</h3>
          ...
          <p className="m-4">
            <FloatingSelect label="Nouvel utilisateur" name="nouvelUtilisateur" required>
              {agents.map((agent, index) =>
                agent.matricule !== materiel.userActuel.matricule && (
...
export default ReaffectMateriel;
